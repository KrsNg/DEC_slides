...
\newline \newline
Simulation technique: Large eddy simulation (LES): higher accuracy than Reynolds averaged Navier Stokes (RANS) $\rightarrow$ lower cost (time, resources) than direct numerical simulation (DNS).
...
{\includegraphics[width=0.6\textheight]{./figs/Temp_DNS_LES2.png}}
\end{figure}
\end{enumerate}
\end{frame}
...
Computational domain $ = 3D $ cuboid\newline
...
\vspace{10pt}
\scriptsize
Mean temperature results agreement between LES and DNS at $\frac{x}{H} = 6$. \newline LES values lower than DNS for $\frac{x}{H} = 12$.\newline
Anticipate this to improve for increased mesh resolution in LES grid.
...
\item Williamschen [2013] previously obtained savings of up to 85\% reduction in cell count in 3D
\end{itemize}
\end{exampleblock}
\end{minipage}
...
\begin{enumerate}[1.]
\item a priori error estimators:  predict long-term behavior of the errors. 
\item a posteriori error estimators: use simulation results to derive estimates of solution errors and guide adaptive schemes. 
\item Where calculated error estimates are higher than set threshold:
\begin{itemize}
\scriptsize
\item Mesh can be locally refined ($h$-refinement) while coarsening the less critical areas to save on computational cost. 
\item Degree of polynomial representation can be raised ($p$-increment)
\end{itemize}
\item Two main \textit{a-posteriori} approaches are:
\begin{enumerate}[(a)]
\scriptsize
...
\begin{frame}%[allowframebreaks]
\scriptsize
\frametitle{Background on gradient/physics-based refinement}
...
\item Main disadvantages of the gradient based approach it the lack of continual reduction of error despite continued $h$ refinements
\end{enumerate}
\end{frame}
...
\begin{enumerate}[(a)]
\scriptsize
\item Make error estimation more relevant to engineering applications
